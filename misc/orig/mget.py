...
class http_client(asyncore.dispatcher):
    ...
    def __init__(self, host, path, parts, pbegin=0, pend=0, m=None):
        ...
        # Otherwise, we are a child, skip the header and download our segment.
            ...
            self.recvhead = 2
            self.buffer = "GET {0} HTTP/1.1\r\nHost: {1}\r\nRange: bytes={2}-{3}\r\n\r\n".format(self.path, self.host, self.pbegin, self.pend)
            print(self.buffer)
        ...
        # Stage 2, clip the second incoming header and start grabbing the file itself.
            ...
            if size > 0:
                ...
                if len(xrange(size/1024)) == 0:
                    ...
                    print("Jet {0}-{1}   {2} of {3} bytes recieved".format(self.pbegin, self.pend, self.bytes, self.length))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
       ...
       client.m.close()
       client.f.close()
       print("Client jet 1 finished at {0} of {1}".format(client.bytes, client.length))
